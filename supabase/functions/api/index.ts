...
app.get("workflows/:id", async (c) => {
  ...
  return c.json(await getWorkflowByIdHandler({ mastra, workflowId: id }));
});

app.get("workflows/:id/runs", async (c) => {
  const id = c.req.param("id");

  return c.json(await getWorkflowRunsHandler({ mastra, workflowId: id }));
});
...
app.get("workflows/:id/watch", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Failed to watch workflow" }, 500);
  }
});
